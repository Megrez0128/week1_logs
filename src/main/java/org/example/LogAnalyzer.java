...
public class LogAnalyzer {
	...
	public String query(Integer tmpSvrId, String vUserID, String vRoleID, Integer mainReason) {
		...
		if (stringMapMap == null) {
			return String.format("Server ", tmpSvrId, " doesn't exist!");
		}
		stringMapMap1 = stringMapMap.get(vUserID);
		if (stringMapMap1 == null) {
			return String.format("User ", vUserID, " doesn't exist in server ", tmpSvrId, "!");
		}
		integerMapMap = stringMapMap1.get(vRoleID);
		if (integerMapMap == null) {
			return String.format("Role ", vRoleID, " doesn't exist under UserID ", vUserID, " in Server ", tmpSvrId, "!");
		}
		Map<String, String> map = integerMapMap.get(mainReason);
		if (map == null) {
			return String.format("Reason ", mainReason, " doesn't exist under RoleID ", vRoleID, " under UserID ", vUserID, " in Server ", tmpSvrId, "!");
		}
		return String.format("totalAddedValue: ", map.get("totalAddedValue"), "  totalConsumptionValue: ", map.get("totalConsumptionValue"));
	}
}
